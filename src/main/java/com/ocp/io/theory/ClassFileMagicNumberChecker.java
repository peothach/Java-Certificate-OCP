...
public class ClassFileMagicNumberChecker {
    ...
    public static void main(String[] args) {
        ...
        // you need to have explicit down cast to byte since
        ...
        try (FileInputStream fis = new FileInputStream(fileName)) {
            ...
            // use a temporary to read first four bytes
            byte[] u4buffer = new byte[4];
            //read  buffer full (4 bytes here) of data from the file
            if (fis.read(u4buffer) != -1) {
                // if read was successful
                // the overload method equals fo two byte arrays
